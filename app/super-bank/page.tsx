...
export default function SuperBankPage() {
  ...
  // Apply tag to all transactions where selection text is present in Descriptions (flat structure)
  const handleApplyTagToAll = async () => {
    if (!pendingTag) return;
    const { tagName, selectionText } = pendingTag;
    const tagObj = allTags.find(t => t.name === tagName);
    if (!tagObj) return setPendingTag(null);
    const descCol = superHeader.find(h => h.toLowerCase().includes('description'));
    if (!descCol) return setPendingTag(null);
    await Promise.all(filteredRows.map(async (row) => {
      if (!row.id) return;
      const val = row[descCol];
      if (typeof val === 'string' && val.toLowerCase().includes(selectionText.toLowerCase())) {
        const tx = transactions.find(t => t.id === row.id);
        if (!tx) return;
        const tags = Array.isArray(tx.tags) ? [...tx.tags] : [];
        if (!tags.some((t) => t.id === tagObj.id)) tags.push(tagObj);
