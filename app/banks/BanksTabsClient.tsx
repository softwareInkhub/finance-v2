...
import { RiBankLine, RiPriceTag3Line, RiEdit2Line, RiDeleteBin6Line } from 'react-icons/ri';
...
export default function BanksTabsClient() {
  ...
  const handleCreateBank = async (bankName: string, tags: string[]) => {
    ...
    try {
      ...
      // Add tab and navigate to new bank
      addTab({
        key: `accounts-${newBank.id}`,
        label: newBank.bankName,
        type: 'accounts',
        bankId: newBank.id,
        href: `/banks/accounts?type=accounts&bankId=${newBank.id}`,
      });
      router.push(`/banks/accounts?type=accounts&bankId=${newBank.id}`);
    } catch (error) {
