...
export default function BanksPage() {
  ...
  const handleBankCardClick = (bank: { id: string; bankName: string }) => {
  ...
  const handleCloseTab = (tabKey: string, e: React.MouseEvent) => {
    ...
    if (tabKey === 'overview') return; // Don't allow closing overview tab
    
    const newTabs = tabs.filter(tab => tab.key !== tabKey);
    setTabs(newTabs);
    
    // If the closed tab was active, switch to the overview tab
    if (activeTab === tabKey) {
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br  from-blue-50 via-white to-purple-50">
      <BanksSidebar onSuperBankClick={handleSuperBankClick} onAccountClick={handleAccountClick} />
        ...
        {activeTab !== 'overview' && (
          (() => {
            ...
            if (tab?.type === 'accounts' && tab.bankId) {
              return <AccountsPage bankId={tab.bankId} onAccountClick={handleAccountClick} />;
            }
            if (tab?.type === 'statements' && tab.bankId && tab.accountId) {
              return <StatementsPage bankId={tab.bankId} accountId={tab.accountId} />;
            }
            return <div className="p-8 text-gray-400 text-center">New Tab Content (customize as needed)</div>;
          })()
        )}
      </main>
    </div>
